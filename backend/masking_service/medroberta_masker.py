"""MedRoBERTa masker (model version ``medroberta-nl-2``).

Wraps the project's fine-tuned Dutch clinical PII detector
(``backend/src/detection``: Presidio + the ``farahelmashad/pii-medroberta-nl-v2``
...
Model source resolution (handled by ``src.detection.transformers_recognizer``):
    ...
    * otherwise the Hugging Face Hub repo ``farahelmashad/pii-medroberta-nl-v2``.
"""
