...
def get_dutch_regex_recognizers():
    ...
    # 1. Date of Birth (Leaked in Doc 3)
    ...
    height_recognizer = PatternRecognizer(supported_language='nl', 
        supported_entity="LEAKED_HEIGHT", 
    ...
    weight_recognizer = PatternRecognizer(supported_language='nl', 
        supported_entity="LEAKED_WEIGHT", 
    ...
    recognizers.append(PatternRecognizer(supported_language='nl', supported_entity="LEAKED_BMI", patterns=[bmi_pattern]))
    ...
    recognizers.append(PatternRecognizer(supported_language='nl', supported_entity="LEAKED_HOSPITAL", patterns=[hospital_pattern]))
    
    # 9. Standard Belgian INSZ
    insz_pattern = Pattern(name="insz_standard", regex=r"\b\d{2}[.-]?\d{2}[.-]?\d{2}[- ]?\d{3}[.-]?\d{2}\b", score=0.8)
    recognizers.append(PatternRecognizer(supported_language='nl', supported_entity="BELGIAN_INSZ", patterns=[insz_pattern]))
    
    # 9b. Standard Belgian RIZIV
    riziv_pattern = Pattern(name="riziv_standard", regex=r"\b\d{1}[-]?\d{5}[-]?\d{2}[-]?\d{3}\b", score=0.8)
    recognizers.append(PatternRecognizer(supported_language='nl', supported_entity="BELGIAN_RIZIV", patterns=[riziv_pattern]))
    
    # 10. Phone (BE/NL)
    phone_pattern = Pattern(name="phone_standard", regex=r"\b(?:0|\+32|\+31)[\s-]?\d[\s-]?\d{6,8}\b", score=0.7)
    recognizers.append(PatternRecognizer(supported_language='nl', supported_entity="PHONE_NUMBER_NL_BE", patterns=[phone_pattern]))
    
    # 11. Dutch/Belgian IBAN
    iban_pattern = Pattern(name="iban_standard", regex=r"\b(?:NL|BE)\d{2}\s?[A-Z]{4}\s?\d{4}\s?\d{4}\s?\d{0,4}\b", score=0.9)
    recognizers.append(PatternRecognizer(supported_language='nl', supported_entity="IBAN_NL_BE", patterns=[iban_pattern]))
