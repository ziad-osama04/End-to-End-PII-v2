...
# Monkeypatch os.symlink to prevent WinError 1314 when downloading HF models on Windows without Admin privileges
_orig_symlink = getattr(os, "symlink", None)
def fake_symlink(src, dst, *args, **kwargs):
    try:
        # Resolve relative src paths relative to the destination directory
        ...
        print(f"Fallback copy failed for {src} -> {dst}: {e}")
os.symlink = fake_symlink

def main():
    print("Loading GLiNER Model... (this may take a few seconds)")
    try:
        from gliner import GLiNER
    except ImportError:
        print("Please install gliner first: pip install gliner")
        return
        
    model = GLiNER.from_pretrained("urchade/gliner_multi-v2.1")
    labels = ["patient name", "doctor name", "hospital", "ID number", "belgian INSZ", "belgian RIZIV"]
    ...
    print("\n" + "="*30)
    print(f"--- GLiNER EVALUATION METRICS (Character-Level) ---")
    print(f"Model: urchade/gliner_multi-v2.1")
    print("="*30)
