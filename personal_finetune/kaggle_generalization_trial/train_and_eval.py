...
TRIAL_ID = os.environ.get("TRIAL_ID", "gen-holdout-batch_final")
...
# Cross-source holdout: a random doc-level split (the Stage 1 approach) only
# proves generalization to unseen DOCUMENTS from the SAME synthetic generator
# style. new_data actually contains 4 structurally distinct generators
# (batch_final / labeled / hf / legacy -- different field-naming conventions,
# different phrasing/templates). Holding one out ENTIRELY from training and
# evaluating purely on it is a genuine test of generalization to a different
# structure, not just a different document.
HELD_OUT_SOURCE = os.environ.get("HELD_OUT_SOURCE", "batch_final")
if HELD_OUT_SOURCE:
    eval_recs = [r for r in records if r["source"] == HELD_OUT_SOURCE]
    train_recs_full = [r for r in records if r["source"] != HELD_OUT_SOURCE]
    rng = random.Random(SEED)
    rng.shuffle(eval_recs)
    rng.shuffle(train_recs_full)
    print(f"Cross-source holdout: eval=ALL of source={HELD_OUT_SOURCE!r} ({len(eval_recs)} docs), "
          f"train=every other source ({len(train_recs_full)} docs)")
else:
    rng = random.Random(SEED)
    shuffled = records[:]
...
if DOC_LIMIT:
    eval_recs = eval_recs[:max(1, DOC_LIMIT // 6)]
    train_recs_full = train_recs_full[:DOC_LIMIT]
    print(f"DRY RUN: capped corpus to train={len(train_recs_full)} eval={len(eval_recs)} docs")
...
with open(os.path.join(REPORT_DIR, "trial_config.json"), "w") as f:
    json.dump({"trial_id": TRIAL_ID, "address_mode": ADDRESS_MODE, "seed": SEED,
               "held_out_source": HELD_OUT_SOURCE, "model_labels": model_labels,
               "max_steps": MAX_STEPS, "n_train": len(train_recs), "n_eval": len(eval_recs)}, f, indent=2)
