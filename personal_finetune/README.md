...
- **Model**: `farahelmashad/pii-tamerbert-v5` on HF Hub (private). This is
  the final checkpoint of this track -- ask Farah for read access if you
  need to pull it. Earlier checkpoints (`v1`-`v4`) are kept there too for
  comparison.
...
- **Quantized (ONNX INT8) model**: `farahelmashad/pii-tamerbert-v5-int8-onnx`
  on HF Hub, built by `kaggle_quantize/quantize_and_eval.py`.
