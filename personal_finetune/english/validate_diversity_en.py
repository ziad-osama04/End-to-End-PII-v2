#!/usr/bin/env python3
"""
validate_diversity_en.py -- measures whether the dataset is ACTUALLY diverse,
not just superficially varied. Direct fork of
personal_finetune/french/validate_diversity_fr.py (language-agnostic
mechanics) -- this is the single check that caught the INSZ/RIZIV
Flemish-vs-French mislabeling bug in the French build that no other
validation layer found, so it's run here against the FIRST draft rather
than deferred to a later polish pass.
...
def section1_lexical(train_docs):
    print("=" * 90)
    print("1. LEXICAL DIVERSITY (train_en.jsonl, full corpus)")
    print("=" * 90)
    all_tokens = []
    for d in train_docs:
...
def section2_vocab_overlap(train_docs, heldout_docs):
    ...
    print(f"  held-out word coverage by train vocab: {coverage:.4f}  "
          f"(expect HIGH -- same clinical English language, this is fine and expected)")
...
def main():
    train_docs = load_jsonl("train_en.jsonl")
    heldout_docs = load_jsonl("tamerbert_data_en/eval_heldout_templates.jsonl")
    ...
    if os.path.exists("train_augmented_en.jsonl"):
        print("=" * 90)
        print("3b. POSITIONAL DIVERSITY ON train_augmented_en.jsonl (post id_label_variant)")
        print("=" * 90)
        aug_docs = load_jsonl("train_augmented_en.jsonl")
        section3_positional_diversity(aug_docs)
    else:
        print("(train_augmented_en.jsonl not built yet -- skipping section 3b, re-run after augment_en.py)")
