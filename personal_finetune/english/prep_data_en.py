#!/usr/bin/env python3
"""
prep_data_en.py -- English fork of personal_finetune/french/prep_data_fr.py.
...
Keeps the Dutch (DD-MM-YYYY/MM/YYYY) and French (JJ-MM-AAAA/MM/AAAA) leak
patterns as defense-in-depth (the English templates were translated from
the same recovered-Dutch source, so a literal leftover format-hint from an
earlier recovery pass is exactly the kind of authoring mistake this filter
exists to catch, even though none is expected).

Usage:
    python3 prep_data_en.py --input train_en.jsonl --out-dir tamerbert_data_en
"""
...
def main():
    ...
    ap.add_argument("--input", default="train_en.jsonl")
    ap.add_argument("--out-dir", default="tamerbert_data_en")
    args = ap.parse_args()
