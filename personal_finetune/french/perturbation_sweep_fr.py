"""
perturbation_sweep_fr.py -- French port of
...
all eval_real_fr.jsonl), each transform correctly recomputing span offsets.
Transform logic ported unchanged (position/offset mechanics are
language-agnostic) -- only the source files are French.

Every transform is (text, spans) -> (new_text, new_spans):
  - length-preserving (casing): spans untouched, just re-sliced.
  - insertion-based (whitespace jitter, punctuation swaps): sorted edit list
    + monotonic offset remapper.
  - truncation: spans fully inside the kept prefix survive; anything
    straddling or past the cut point is dropped (correctly absent from gold,
    since it's genuinely not recoverable from a truncated document).
"""
import json
import os
import random
...
if __name__ == "__main__":
    ...
    heldout = load(os.path.join(HERE, "tamerbert_data_fr", "eval_heldout_templates.jsonl"))
    real = load(os.path.join(HERE, "eval_real_fr.jsonl"))
    ...
    out_path = os.path.join(HERE, "perturbation_sweep_fr.jsonl")
