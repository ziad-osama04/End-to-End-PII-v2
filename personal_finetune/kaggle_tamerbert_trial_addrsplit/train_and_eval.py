...
TRIAL_ID = os.environ.get("TRIAL_ID", "tamerbert-addrsplit-v1")
...
# train/indist/heldout_templates come from pii_table.py generation, where
# every historical ADDRESS string was verified to follow the exact regular
# format "<street>, <4-digit zip> <city>" with zero exceptions -- so these
# three were split into STREET/ZIPCODE/CITY offline (split_address.py) with
# no loss of information, equivalent to having generated with a "split"
# label scheme in the first place. real/ood were NOT generated by
# pii_table.py and aren't guaranteed to follow that format, so their gold
# stays single-ADDRESS; this model's STREET/ZIPCODE/CITY predictions get
# remapped+merged back into ADDRESS before scoring against them (below).
train_recs_full = load_split("train_filtered_addrsplit.jsonl")
eval_sets = {
    "indist": load_split("eval_indist_addrsplit.jsonl"),
    "heldout_templates": load_split("eval_heldout_templates_addrsplit.jsonl"),
    "real": load_split("eval_real.jsonl"),
    "ood": load_split("ood_docs.jsonl"),
}
ADDRESS_COMPONENT_LABELS = {"STREET", "ZIPCODE", "CITY"}
SINGLE_ADDRESS_SURFACES = {"real", "ood"}

if DOC_LIMIT:
    train_recs_full = train_recs_full[:DOC_LIMIT]
    eval_sets = {k: v[:max(1, DOC_LIMIT // 10)] for k, v in eval_sets.items()}
    print(f"DRY RUN: capped train={len(train_recs_full)}  " +
...
print(f"train={len(train_recs)}  " + "  ".join(f"{k}={len(v)}" for k, v in eval_sets.items()))
...
with open(os.path.join(REPORT_DIR, "trial_config.json"), "w") as f:
    json.dump({"trial_id": TRIAL_ID, "seed": SEED, "model_labels": model_labels,
               "max_steps": MAX_STEPS, "n_train": len(train_recs),
...
if SKIP_TRAINING:
    assert PRETRAINED_MODEL_DIR and os.path.isdir(PRETRAINED_MODEL_DIR), \
        "SKIP_TRAINING=1 requires PRETRAINED_MODEL_DIR to point at a saved final_model/ dir"
    print(f"SKIP_TRAINING: loading finished model from {PRETRAINED_MODEL_DIR} -- no GPU training this run.")
    ...
    print(f"Saved final model to {FINAL_MODEL_DIR} (persists as this kernel version's output).")
...
def remap_address_components(spans, gap=3):
    """STREET/ZIPCODE/CITY -> ADDRESS, then merge same-label spans separated
    by at most `gap` chars (covers the ", " / " " literal joins) into one
    contiguous span, matching how real/ood's gold ADDRESS is annotated."""
    relabeled = sorted(
        ((s, e, "ADDRESS" if lab in ADDRESS_COMPONENT_LABELS else lab) for s, e, lab in spans),
        key=lambda x: x[0])
    merged = []
    for s, e, lab in relabeled:
        if merged and merged[-1][2] == lab == "ADDRESS" and s - merged[-1][1] <= gap:
...
model_gold_by_set = {}
for name, recs in eval_sets.items():
    score_labels = ((set(model_labels) - ADDRESS_COMPONENT_LABELS) | {"ADDRESS"}) \
        if name in SINGLE_ADDRESS_SURFACES else set(model_labels)
    model_gold_by_set[name] = [[(e["start"], e["end"], e["label"]) for e in r["entities"] if e["label"] in score_labels]
                                for r in recs]
...
for name, recs in eval_sets.items():
    ...
    if name in SINGLE_ADDRESS_SURFACES:
        model_preds = [remap_address_components(doc_spans) for doc_spans in model_preds]
    ...
    # Native STREET/ZIPCODE/CITY report answers "how well is each component
    # learned"; this additional remapped-and-merged-back-to-ADDRESS report
    # answers "how does end-to-end address extraction compare to the
    # single-ADDRESS baseline" -- the actual ablation question. No-op
    # re-merge for real/ood (already ADDRESS-only), a real remap for
    # indist/heldout_templates.
    model_gold_addrmerged = [remap_address_components(g) for g in model_gold]
    model_preds_addrmerged = [remap_address_components(p) for p in model_preds]

    reports = {}
    for mode in ("strict", "relaxed"):
        reports[f"model_only_{mode}"] = run_eval(model_gold, model_preds, "model_only", mode)
        reports[f"model_only_addrmerged_{mode}"] = run_eval(
            model_gold_addrmerged, model_preds_addrmerged, "model_only_addrmerged", mode)
