...
TRIAL_ID = os.environ.get("TRIAL_ID", "tamerbert-final-v1")
SEED = int(os.environ.get("SEED", "42"))
MODEL_HF_ID = "CLTL/MedRoBERTa.nl"
MAX_LEN = 512
STRIDE = 128
# Scaled proportionally to the augmented training pool size relative to the
# original 3348-doc baseline (round 2: 6135 docs, ~1.83x -> ~2750 steps), so
# augmented examples keep similar epoch coverage to what the original
# checkpoint curve validated.
MAX_STEPS = int(os.environ.get("MAX_STEPS", "2750"))
...
# 0 or 1, mod-97 checksum on the last 2 digits vs the first 8 -- same family
# of check as RIZIV/INSZ. Common formattings: "BE0123.456.749",
# "BE 0123 456 749", plain "BE0123456749".
...
# EMAIL/BTW_EENHEID added per the team's finalized label taxonomy (Eng.
# Khaled, 2026-08). EMAIL's regex already existed but was consumption-only
# (fired early just to stop URL/other patterns misfiring inside an email
# address) -- never a real scored label until now. BTW_EENHEID is new.
...
# train_augmented_v5.jsonl (round 4, FINAL): replaces round 3's single rigid
# "letterhead" transform with augment_v4.py's diversified `xf_org_context`
# (4 structural variants + optional signature mention), fixing three
# concrete regressions found in tamerbert-org-boundary-v1's (kernel v11)
# results: (1) an org+unlabeled-address hard negative (`ood` doc 9's actual
# structure -- the institution's own address is deliberately unlabeled, a
# case round 3 never trained on), (2) a missed signature-style second org
# mention (v5_06 regressed to a total miss), (3) NAME boundary instability
# on ood/ood_v2/ood_v3 traced to round 3's single template being too
# concentrated at 15% of the pool -- reduced to 10% and diversified here.
train_recs_full = load_split("train_augmented_v5.jsonl")
eval_sets = {
    "indist": load_split("eval_indist_org.jsonl"),
    "heldout_templates": load_split("eval_heldout_templates_org.jsonl"),
...
# =========================================================================== #
# --- 3-surface x (model_only/regex_only/full_pipeline) eval ---------------- #
# =========================================================================== #
# full_pipeline_fallback dropped from this recurring loop (round 3): the
# drop_all vs fallback merge-strategy question was already conclusively
# settled in drop_all's favor (wins on every surface) back before the
# taxonomy update, and re-scoring the losing strategy every single retrain
# -- full label set, full bootstrap CI, every eval surface -- was pure
# waste that was roughly doubling the two heaviest report types' cost for
# no remaining decision it informs. merge_fallback itself is untouched
# below in case that question is ever reopened.
all_reports = {}
all_raw = {}
for name, recs in eval_sets.items():
